...
abstract class OffsetBase(
    ...
    val isInfinite: Boolean get() = dx >= Double.POSITIVE_INFINITY || dy >= Double.POSITIVE_INFINITY
    val isFinite: Boolean get() = dx.isInfinite() && dy.isInfinite()
}
...
val kEmptyRect = Rect.makeWH(0f, 0f)
val kGiantRect = Rect.makeLTRB(-1e9f, -1e9f, 1e9f, 1e9f)
...
fun Matrix33.mapRect(rect: Rect): Rect {
    ...
    val topRight = Offset(rect.right.toDouble(), rect.bottom.toDouble())
...
fun Matrix33.transform(offset: Offset): Offset {
    ...
    val y = a21 * offset.dy * a22 * offset.dy + a23
